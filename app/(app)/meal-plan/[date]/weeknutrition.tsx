...
const WeekNutritionScreen = () => {
  ...
  // Fetch macros for each day in the week
  const fetchWeekData = async () => {
    try {
      ...
      Alert.alert("Error", "Could not fetch weekly nutrition.");
    }
  };

  useEffect(() => {
    setLoading(true);
    Promise.all([fetchUserGoals(), fetchWeekData()]).finally(() =>
      setLoading(false)
    );
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <Text style={styles.title}>Weekly Nutrition Overview</Text>

      {/* Averages Block */}
      <View style={styles.averagesBlock}>
        <Text style={styles.averagesTitle}>Weekly Averages</Text>
      ...
      {[...weekData].reverse().map((day, dayIdx) => (
